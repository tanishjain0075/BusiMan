...
const AddClient = ({ existing, onSuccess, onCancel }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (isEdit) {
        await updateClient(existing._id, payload);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input {...register('name', { required: 'Required' })} placeholder="John Doe" className={inputCls} />
          {errors.name && <p className={errorCls}>{errors.name.message}</p>}
        </div>
        <div>
          <label className={labelCls}>Email</label>
          <input type="email" {...register('email')} placeholder="client@example.com" className={inputCls} />
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input {...register('address.city')} placeholder="Mumbai" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>State</label>
              <input {...register('address.state')} placeholder="Maharashtra" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Pincode</label>
              <input {...register('address.pincode')} placeholder="400001" className={inputCls} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button type="submit" loading={submitting}>{isEdit ? 'Save Changes' : 'Add Client'}</Button>
      </div>
    </form>
  );
};
